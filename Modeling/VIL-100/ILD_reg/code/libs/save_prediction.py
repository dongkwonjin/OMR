...
class Save_Prediction_for_eval_iou(object):
    ...
    def rescale_pts(self, pts):
        pts[:, :, 0] = pts[:, :, 0] / (self.cfg.width - 1) * (self.org_w - 1)
        pts[:, :, 1] = (pts[:, :, 1] + self.cfg.crop_size) / (self.cfg.height + self.cfg.crop_size - 1) * (self.org_h - 1)
        return pts
    ...
    def get_2D_lane_points(self, px_coord):
        ...
        for i in range(len(lane_pts)):
            ...
            try:
                self.pred_lane.append(lane_pts[i, height_idx[0]:height_idx[1]])
            except:
                self.pred_lane.append(lane_pts[i])

    def load_gt_data(self):
        if 'train' in self.test_mode:
            data = load_pickle(f'{self.cfg.dir["pre0_train"]}/{self.file_name}')
        else:
            data = load_pickle(f'{self.cfg.dir["pre0_test"]}/{self.file_name}')
        self.org_h = data['height']
        self.org_w = data['width']

        self.shape_list.append((self.org_h, self.org_w, 3))
    ...
    def run(self):
        ...
        txt_path = f'{self.cfg.dir["out"]}/{self.test_mode}/results/txt/pred_txt/datalist.txt'
        ...
        save_pickle(f'{self.cfg.dir["out"]}/{self.test_mode}/results/shape_list', self.shape_list)
    ...
    def settings(self, key, test_mode='test', use_height=True):
        ...
        self.output_dir = f'{self.cfg.dir["out"]}/{test_mode}/results/txt/pred_txt'
