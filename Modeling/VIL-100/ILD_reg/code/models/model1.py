...
class Model(nn.Module):
    def __init__(self, cfg):
        ...
        self.backbone = resnet(layers=self.cfg.backbone, pretrained=True)
    ...
    def forward_for_classification_occ(self):
        ...
        self.occ_prob_map_logit = self.classifier_occ(self.img_feat)
        self.occ_prob_map = F.softmax(self.occ_prob_map_logit, dim=1)[:, 1:2]

        return {'occ_prob_map_logit': self.occ_prob_map_logit,
                'occ_prob_map': self.occ_prob_map}
