...
class Test_Process(object):
    def __init__(self, cfg, dict_DB):
        ...
        self.eval_iou_official = dict_DB['eval_iou_official']
    ...
    def evaluation(self, mode):
        ...
        if bool(self.cfg.do_eval_iou_official + self.cfg.do_eval_iou_laneatt) == True:
            self.save_pred_for_eval_iou.settings(key=['x_coords'], test_mode=mode, use_height=True)
            self.save_pred_for_eval_iou.run()
            if self.cfg.do_eval_iou_official == True:
                metric.update(self.eval_iou_official.measure_IoU(mode, iou=self.cfg.iou_thresd['official']))
