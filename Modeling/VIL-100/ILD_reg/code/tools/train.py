...
class Train_Process(object):
    ...
    def training(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            outputs.update(self.model1.forward_for_classification())
            outputs.update(self.model1.forward_for_classification_occ())
            outputs.update(self.model.forward_for_regression(self.model1.prob_map))
            ...
            if (self.epoch > 100 or (self.epoch + 1) % 10 == 0) and (self.batch_iteration % self.cfg.iteration['validation'] == 0):
                self.test()
                self.model.train()
    ...
    def test(self):
        metric = self.test_process.run(self.model1, self.model, mode='val')
