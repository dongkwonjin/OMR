...
def load_model_for_test(cfg, dict_DB):
    ...
    if cfg.run_mode == 'test_paper':
        checkpoint = torch.load(f'{cfg.dir["weight_paper"]}/checkpoint_max_F1_vil100_OMR')
    else:
...
def load_pretrained_model1(cfg):
    ...
    if cfg.run_mode == 'test_paper':
        checkpoint = torch.load(f'{cfg.dir["weight_paper"]}/checkpoint_max_seg_fscore_vil100_ILD_cls')
...
def load_pretrained_model2(cfg):
    ...
    if cfg.run_mode == 'test_paper':
        checkpoint = torch.load(f'{cfg.dir["weight_paper"]}/checkpoint_max_F1_vil100_ILD_reg')
