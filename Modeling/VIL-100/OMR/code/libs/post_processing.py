    ...
    def nms(self):
        ...
        for i in range(self.cfg.max_lane_num * 2):
            ...
            if prob_map[idx_max // w, idx_max % w] > self.cfg.nms_thresd:
                ...
                lane_mask = self.draw_polyline_cv(to_np(lane_coords), color=(1, 1, 1), s=self.cfg.removal['lane_width'] + 1)
                lane_mask2 = self.draw_polyline_cv(to_np(lane_coords), color=(1, 1, 1), s=1)
                score = self.measure_confidence_score(prob_map2, lane_mask2)
