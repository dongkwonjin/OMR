...
class Transforms(object):
    ...
    def settings(self):
        transforms = self.custom_transforms(self.cfg.height + self.cfg.crop_size, self.cfg.width)
        transforms_for_test = self.transforms_for_test(self.cfg.height + self.cfg.crop_size, self.cfg.width)
        transforms_for_syn_generation = self.transforms_for_syn_generation(self.cfg.height, self.cfg.width)
        ...
        img_transforms_for_syn_generation = []
        for aug in transforms_for_syn_generation:
            p = aug['p']
            img_transforms_for_syn_generation.append(
                iaa.Sometimes(p=p, then_list=getattr(iaa, aug['name'])(**aug['parameters'])))

        self.transform = iaa.Sequential(img_transforms)
        self.transform_for_test = iaa.Sequential(img_transforms_for_test)
        self.transform_for_syn_generation = iaa.Sequential(img_transforms_for_syn_generation)

    def lane_to_linestrings_syn(self, data):
        lane = list()
    ...
    def process_anno_only(self, img_org, anno):
        line_strings_org = self.lane_to_linestrings(anno)
        line_strings_org = LineStringsOnImage(line_strings_org, shape=(img_org.height, img_org.width, 3))
        line_strings = self.transform_for_test(line_strings=line_strings_org)
        anno_new = self.linestrings_to_lanes(line_strings)
        return anno_new

    def process_for_syn_generation(self, img_org, anno):
        img_org = np.uint8(img_org)
        line_strings_org = self.lane_to_linestrings_syn(anno)
        line_strings_org = LineStringsOnImage(line_strings_org, shape=img_org.shape)
        img_new, line_strings = self.transform_for_syn_generation(image=img_org, line_strings=line_strings_org)
        anno_new = self.linestrings_to_lanes(line_strings)
        return img_new, anno_new
    ...
    def process_for_test_anno_only(self, img_org, anno):
        line_strings_org = self.lane_to_linestrings(anno)
        line_strings_org = LineStringsOnImage(line_strings_org, shape=(img_org.height, img_org.width, 3))
        line_strings = self.transform_for_test(line_strings=line_strings_org)
        anno_new = self.linestrings_to_lanes(line_strings)
    ...
    def transforms_for_syn_generation(self, img_h, img_w):
        transform = [
            dict(name='Resize',
                 parameters=dict(size=dict(height=img_h, width=img_w)),
                 p=1.0),
