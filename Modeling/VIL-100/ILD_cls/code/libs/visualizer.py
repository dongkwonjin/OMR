...
class Visualize_cv(object):
    ...
    def display_for_training(self, batch, out, batch_idx):
        ...
        # self.show['coeff_pos_map'] = self.overlap_lane_coord_map(lane_coord_map, out['prob_map'][0])

        self.show['occ_prob_map'] = self.b_map_to_rgb_image(out['occ_prob_map'][0])
        self.show['occ_label'] = self.b_map_to_rgb_image(out['obj_mask'][0] > 0.5)

        save_namelist = ['img', 'prob_map', 'coeff_pos_map', 'org_label', 'occ_prob_map', 'occ_label']

        # save result
        self.display_imglist(path=f'{self.cfg.dir["out"]}/train/display/{str(batch_idx)}.jpg', list=save_namelist)
    ...
    def display_for_test(self, batch, out, batch_idx, mode):
        ...
        # output
        self.show['prob_map'] = self.b_map_to_rgb_image(out['prob_map'][batch_idx])
        ...
        self.show['occ_prob_map'] = self.b_map_to_rgb_image(out['occ_prob_map'][batch_idx])
        self.show['occ_label'] = self.b_map_to_rgb_image(out['obj_mask'][batch_idx] > 0.5)

        self.show['img_feat'] = self.feat_map_to_rgb_image(out['img_feat'][batch_idx])
        ...
        self.display_imglist(path=f'{self.cfg.dir["out"]}/{mode}/display/{dirname}/{filename}.jpg',
                             list=['img', 'prob_map', 'seg_overlap', 'label_overlap', 'org_label', 'occ_prob_map', 'occ_label', 'img_feat'])
