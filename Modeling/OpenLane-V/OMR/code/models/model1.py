...
class Pixel_Decoder(torch.nn.Module):
    def __init__(self, cfg):
        ...
        # self.num_decoders = cfg.num_layers['dec']
        self.num_decoders = 1
    ...
    def forward(self, features):
        ...
        out = self.feat_combine(x_concat)
        return out
...
class Model(nn.Module):
    def __init__(self, cfg):
        ...
        # backbone -> Pixel encoder
        self.backbone = Backbone(cfg.backbone, True, True, False)

        # Pixel decoder
        self.pixel_decoder = Pixel_Decoder(cfg)
        ...
        self.classifier_occ = nn.Sequential(
            Conv_Norm_Act(self.c_dims, self.c_dims, kernel_size=1, conv_type='2d'),
            Conv_Norm_Act(self.c_dims, self.c_dims, kernel_size=1, conv_type='2d'),
            nn.Conv2d(self.c_dims, 2, kernel_size=1),
        )
    ...
    def forward_for_feature_extraction(self, img):
        # Feature extraction
        self.features = self.backbone(img)
        self.img_feat = self.pixel_decoder(self.features)

    def forward_for_classification(self):
        self.prob_map_logit = self.classifier(self.img_feat)
        self.prob_map = F.softmax(self.prob_map_logit, dim=1)[:, 1:2]

        return {'prob_map_logit_init': self.prob_map_logit,
                'prob_map_init': self.prob_map}

    def forward_for_classification_occ(self):
        b, _, _, _ = self.img_feat.shape

        occ_prob_map_logit = self.classifier_occ(self.img_feat)
        self.obj_mask = F.softmax(occ_prob_map_logit, dim=1)[:, 1:2]

        return {'obj_mask': self.obj_mask}

    def forward_for_regression(self):
        # b, _, _, _ = prob_map.shape
        feat_c = self.feat_embedding(self.img_feat)
        ...
        return {'coeff_map_init': self.coeff_map}