...
class Model(nn.Module):
    def __init__(self, cfg):
        ...
        # self.lane_pos_feat_embedding = nn.Sequential(
        #     Conv_Norm_Act(cfg.top_m, self.c_dims, kernel_size=3, padding=1, conv_type='2d'),
        #     Conv_Norm_Act(self.c_dims, self.c_dims, kernel_size=3, padding=1, conv_type='2d'),
        #     nn.Conv2d(self.c_dims, self.c_dims, kernel_size=1),
        # )
        ...
        self.feat_embedding = nn.Sequential(
            Conv_Norm_Act(1, self.c_dims, kernel_size=3, padding=2, dilation=2, conv_type='2d'),
    ...
    def forward_for_data_translation(self, is_training=True):
        ...
        if self.prev_frame_num == 0:
            ...
            res = self.forward_for_ConvLSTM(feat_combined, self.hidden_state, return_gates=False)
    ...
    def forward_for_regression(self, input_tensor):
        feat_c = self.feat_embedding(input_tensor.detach())
