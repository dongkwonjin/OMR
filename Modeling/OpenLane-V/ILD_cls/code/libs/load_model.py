...
from models.model_occ import Model as Model_OCC


def load_model_for_test(cfg, dict_DB):
    dict_DB['model_occ'] = load_pretrained_model3(cfg)
...
def load_model_for_train(cfg, dict_DB):
    ...
    cfg.optim['milestones'] = list(np.arange(0, len(dict_DB['trainloader']) * cfg.epochs, len(dict_DB['trainloader']) * 3))[1:]
...
def load_pretrained_model1(cfg):
    checkpoint = torch.load(f'{cfg.dir["pretrained_weight"]}/checkpoint_max_seg_fscore_{cfg.dataset_name}_{cfg.backbone}')
    model = Model1(cfg=cfg)
    model.load_state_dict(checkpoint['model'], strict=False)
    model.cuda()
    return model

def load_pretrained_model3(cfg):
    model = Model_OCC(cfg=cfg)
    model.cuda()
    return model